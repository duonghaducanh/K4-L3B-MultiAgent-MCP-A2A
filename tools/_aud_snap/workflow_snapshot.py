...
class _Investigation:
    ...
    async def shipment_agent(self) -> None:
        self.trace.emit(
            ...
            decision_code="derive_delivery_timeline",
        )
        self.trace.emit(
            case_id=self.case_id,
            event_type="handoff",
            ...
            evidence_refs=self.ledger.refs_for("get_order", "get_order_items"),
        )
    ...
    def late_actor(self) -> str | None:
        """Who caused a late delivery, from the target order's own dates.

        The seller hands the parcel to the carrier; if that handoff happened
        after the item's shipping limit the seller was late, otherwise the
        parcel reached the carrier in time and the logistics provider owns the
        delay. Derived from the order and item rows so no separate shipment
        read is needed.
        """
        if self.target is None:
            return None
