...
#
# This is also the required evidence set: the five domains every case needs
# (customer timeline, order row, item rows, payment lifecycle, policy rules),
# plus the refund lifecycle for the topics that carry one. Everything the output
# schema needs is derivable from these, so no other read is issued.
...
class _Investigation:
    ...
    async def order_agent(self) -> None:
        ...
        self.trace.emit(
            ...
            evidence_refs=self.ledger.refs_for("get_order_items"),
        )

    async def shipment_agent(self) -> None:
        """Read the carrier handoff timeline, but only when a delivery is late.

        The shipment summary is the authoritative source for *who* caused a late
        delivery, and the output consults that actor only when the target order
        was in fact delivered past its estimate. For an on-time order the read
        would spend an audited call on a domain the verdict never consults.
        """
        self.trace.emit(
            case_id=self.case_id,
            event_type="task_assigned",
        ...
        self.trace.emit(
            ...
            evidence_refs=self.ledger.refs_for(
                "get_shipment_summary", "get_order", "get_order_items"
            ),
        )
    ...
    def late_actor(self) -> str | None:
        """Who caused a late delivery, preferring the shipment timeline.

        The carrier handoff events name the actor directly; when the shipment
        read was skipped (any non-late topic, where the actor is not consulted)
        the target order's own dates answer the same question: if the parcel
        reached the carrier after the item's shipping limit the seller was late,
        otherwise the logistics provider owns the delay.
        """
        ...
        if self.target is None:
            return None
        carrier = _parse(self.target.get("order_delivered_carrier_date"))
        limits = [
            value
            for value in (_parse(item.get("shipping_limit_date")) for item in self.items)
            if value
        ]
        if carrier is None or not limits:
            return None
    ...
    def _claim_evidence(self, topic: str | None, issue: str) -> list[str]:
        ...
        Only the domains that bear on the claim are cited; the irrelevant
        domains are left out so the evidence stays precise.
        """
